...
public:
    ...
    // option parameter allow to exclude devices
    std::vector<std::unique_ptr<LaserdockDevice> > get_laserdock_devices(const std::vector<LaserdockDevice*> &excludedDevices = std::vector<LaserdockDevice*>());
