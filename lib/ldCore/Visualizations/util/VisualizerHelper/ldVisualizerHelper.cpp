...
float BeatSignal::process(float input, float delta) {
...
float PresetSignal::processSignal(float f) {
...
QString MusicReactor::labels[MRPARAMS] = {
...
void MusicReactor::setParams(float _params[]) {
    for (int i = 0; i < MRPARAMS; i++) params[i] = clampf(_params[i], 0, 1);
    updateParams();
}


float MusicReactor::process(ldSoundData* p) {
...
void MusicReactor::processStat(ldSoundData* p, float d)
{
...
void MusicReactor::processSignal(float delta)
{
    signal.mult = fscale(signalMult, 0.f, 4.9f);
    signal.floor = fscale(signalFloor*signalFloor, 0.f, 0.9f);
    signal.fade = fscale(signalFade, 0.f, 0.9f);
...
void MusicReactor::processTracker(float d)
{
    int trackerRangeStartValue = iscale(trackerRangeStart, 6, 30); // 240/2 max
    int trackerRangeEndValue = iscale(trackerRange, trackerRangeStartValue*2, 120); // 120 max
    tracker->set(d*trackerRangeStartValue,
...
void MusicReactor::updateParams() {
    float* p[] = {&outputType,
