...
LaserPoint::LaserPoint(Vertex v){
    x = v.position[0];
    y = v.position[1];
    r = v.color[0];
    g = v.color[1];
...
void LaserPoint::setDataBufferAtLocation(/*ldDataDispatcher::ldChanneledData& data, int i*/) {
   /* data.m_bufferX[i] = x*0.99;
    data.m_bufferY[i] = y*0.99;
    data.m_bufferR[i] = clampf(r, 0.01, 0.99);
    data.m_bufferG[i] = clampf(g, 0.01, 0.99);
    data.m_bufferB[i] = clampf(b, 0.01, 0.99);*/
}
void LaserPoint::getDataBufferAtLocation(/*ldDataDispatcher::ldChanneledData& data, int i*/) {
  /*  x = data.m_bufferX[i];
    y = data.m_bufferY[i];
    r = data.m_bufferR[i];
    g = data.m_bufferG[i];
