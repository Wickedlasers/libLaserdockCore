...
class LDCORESHARED_EXPORT ControlValue {
public:
    char* name;
    float* pointer;
    float min;
    ...
    float increment;
    ControlValue();
    ControlValue(char* _name, float* _pointer, float _min, float _value, float _max, float _increment)
        : name(_name)
        , pointer(_pointer)
        , min(_min)
        , value(_value)
...
inline void addControl(ControlValue cv) {*(cv.pointer) = cv.value;}
