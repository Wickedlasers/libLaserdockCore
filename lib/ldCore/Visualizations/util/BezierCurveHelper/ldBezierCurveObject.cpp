...
{
    m_curves = ldMaths::translateSvgBezierCurves(m_curves, vec2);
...
{
    m_curves = ldMaths::rotateSvgBezierCurves(m_curves, rotateValue);
...
{
    m_curves = ldMaths::scaleSvgBezierCurves(m_curves, scaleValue);
...
{
    for (std::vector<BezierCurve> &curves : m_curves) {
...
void ldBezierCurveObject::setUnitedCoordinates(bool isUnitedCoordinates)
{
    ...
    if(isUnitedCoordinates) {
        m_curves = ldMaths::svgBezierLaserToUnitedCoords(m_curves);
    } else {
        m_curves = ldMaths::svgBezierUnitedToLaserCoords(m_curves);
    }
    resetCache();
}
