...
DurationalStatEstimator::DurationalStatEstimator(float /*seconds*/) {
...
void DurationalStatEstimator::add(float f) {
...
void PitchTracker::update(ldSoundData* psd, float factor, bool transition, float _freq) {
...
SpectAdvanced::SpectAdvanced(float rho) {
...
void SpectAdvanced::update(const ldSpectrumFrame &f) {
...
SigmoidDetector::SigmoidDetector() {
...
void SigmoidDetector::update(float f, float bpf) {
...
void BeatWarm::update(SpectAdvanced* spectAdvanced, SpectAdvanced* spectAdvanced2, float freq1, float freq2) {
...
float BeatFresh::update(const std::vector<float> vect, int sf) {
    for (int i = 0; i < len; i++) {
