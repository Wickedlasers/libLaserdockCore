...
uint32_t HybridFlash::getColor(int i) {
    i %= 16;
    if (!ison[i]) return 0x000000;
    return colors[i];
}

uint32_t HybridFlash::getColor(int index, int max, int limit) {
...
HybridAnima::HybridAnima() {
    outputBaseFrameIndex = 0;
    outputTrackPosition = 0;
    toggle = false;
    lockout = 0;
}

void HybridAnima::process(ldMusicManager* m) {
...
void HybridAutoColor2::process(ldMusicManager* m) {
...
void HybridColorPalette::process(ldMusicManager* m) {
...
void HybridColorPalette::colorize(float& rr, float& gg, float& bb) {
    float r = rr;
    float g = gg;
    float b = bb;
