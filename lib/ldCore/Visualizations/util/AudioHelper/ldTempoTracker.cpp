...
TempoTracker::TempoTracker(char* _algorithm, bool _fastBeats, bool _allowPartialBeats, float _newBeatConfidenceCutoff)
...
float TempoTracker::process(ldSoundData* pSoundData) {
    ...
    if (out->data[0] != 0 && (allowPartialBeats || output <= 0)) {
        /* char s[1024];
        ...
        // set beat
        if (confidence >= newBeatConfidenceCutoff) {
            // activate output
            output = 1;
