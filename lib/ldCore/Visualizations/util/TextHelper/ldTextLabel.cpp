...
// ldTextLabel
ldTextLabel::ldTextLabel(const std::string& p_string, float p_fontSize, const Vec2 &p_position)
    : ldAbstractText()
    , _color(0xFFFFFF)
...
// updateString
void ldTextLabel::setText(const std::string& p_string)
{
...
string ldTextLabel::getText() const
{
    return m_string;
}
...
void ldTextLabel::setPosition(const Vec2 &p_p)
{
    ...
    _position = p_p;
    _labelLettersFrame.translate(Vec2(_position.x - oldPosition.x, _position.y - oldPosition.y));
}
