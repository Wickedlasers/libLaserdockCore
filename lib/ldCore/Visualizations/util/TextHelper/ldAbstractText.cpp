...
#include "ldCore/Visualizations/util/TextHelper/ldTextSvgHelper.h"
...
void ldAbstractText::setText(const string &p_string)
{
...
void ldAbstractText::initLabelLettersFrame(const string &word)
{
    _labelLettersFrame.clear();

    // here in _allSvgLetters
    const float interLetter = getInterLetterWidth();
    const float space = getSpaceWidth();

    // _labelLetters build
    QString qString(word.c_str());
    float letterOffsetX = 0.f;
    float letterOffsetY = 0.f;

    for (const QChar &strChar : qString) {
        if (strChar == ' ') {
            letterOffsetX += space + 2*interLetter;
            continue;
        } else if (strChar == '\t') {
            letterOffsetX += 3*space + 2*interLetter;
            continue;
        } else if (strChar == '\n') {
            letterOffsetX = 0;
        ...
        // next letter offset
        letterOffsetX += letterObject.dim().width() + 2.f*interLetter;
    }
}
...
{
    float interLetter = 0.09f*getLetterAWidth();

    switch (m_fontFamily) {
    case ldFont::Family::Elixia:
         interLetter = 0.25f*getLetterAWidth();
        break;
    default:
        break;
    }
