...
{
    if (p_logo_path != "") {
        ldBezierCurveObject logoSvg2d = ldBezierCurveObject(ldSvgReader::loadSvg(p_logo_path, LD_SVG_READING_MAXIMIZE, 0.01f),
                                                            false);
...
void ldTextCounterOne::setMessage(const QString &message)
{
    ...
    _messageTextLabel->setText(_suffixMessage.toStdString());
...
void ldTextCounterOne::updateString()
{
    ...
    _textLabel->setText(QString::number(countToShow).toStdString());
    // get bezier frame
