        ...
        // everything is ok, process buffer flow
            ...
            if (localBuffer > 0) {
                ...
                if (actualSamplesToSend > 0) {
                    // send
                    if (m_isActive) {
                        QByteArray dataToSend = QByteArray::fromRawData((const char *) compressedSampleVec.data(), actualSamplesToSend*sizeof(CompressedSample));
                        m_usbDeviceManager->sendData(dataToSend);
                    }
