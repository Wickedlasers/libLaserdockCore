...
#include "ldUSBHardwareManager.h"
...
void ldUsbHardwareManager::sendData(QByteArray &data)
{
    QMutexLocker locker(&m_mutex);
    ...
    while(usbHardwareIt != m_usbHardwares.end()) {
        ...
        bool b = usbHardware->send_samples((CompressedSample*)data.data(), data.size() / sizeof(CompressedSample));
        if (!b) {
