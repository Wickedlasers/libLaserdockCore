...
public:
    ...
    bool send_samples(CompressedSample * cdata, unsigned int sample_count){

//        Q_Q(ldUsbHardware);

        return params.device->send_samples((LaserdockSample *) cdata, sample_count);
    }
...
bool ldUSBHardware::send_samples(CompressedSample * cdata, unsigned int sample_count){
    Q_D(ldUSBHardware);
    bool ok = d->send_samples(cdata, sample_count);
    if(!ok) {
